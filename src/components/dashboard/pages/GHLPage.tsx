      ...
      {loading && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 200 }}>
          <div style={{ textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 15, color: 'var(--text3)' }}>Loading GHL data…</div>
          </div>
        </div>
      )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(155px, 1fr))', gap: 12, marginBottom: 20 }}>
            {[
              ...
              <div key={s.label} style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 12, padding: 16, position: 'relative', overflow: 'hidden' }}>
                ...
                <div style={{ fontSize: 13, fontWeight: 600, letterSpacing: '0.07em', textTransform: 'uppercase', color: 'var(--text3)' }}>{s.label}</div>
              </div>
            ))}
          </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: 10 }}>
              {[
                ...
                <div key={stage.label} style={{ background: 'var(--bg3)', borderRadius: 10, padding: '14px 12px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
                  ...
                  <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text3)', letterSpacing: '0.06em', textTransform: 'uppercase' }}>{stage.label}</div>
                </div>
              ))}
            </div>
          </div>
                ...
                {data.conversations.map((c: any) => (
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                      <div style={{ width: 34, height: 34, borderRadius: '50%', background: 'linear-gradient(135deg,#5b7fff,#9f7aea)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'DM Sans, sans-serif', fontSize: 15, fontWeight: 700, color: '#fff', flexShrink: 0 }}>
                        {c.contactName.split(' ').map((w: string) => w[0]).join('').slice(0,2).toUpperCase()}
                      </div>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 15, fontWeight: 600, color: 'var(--text)' }}>{c.contactName}</div>
                      ...
                      <div style={{ display: 'flex', gap: 6 }}>
                        <span style={{ fontSize: 12, fontWeight: 700, padding: '2px 8px', borderRadius: 10, background: c.type === 'TYPE_VOICE' ? 'rgba(240,180,41,0.1)' : 'rgba(91,127,255,0.1)', color: c.type === 'TYPE_VOICE' ? '#f0b429' : '#5b7fff' }}>
                          {c.type === 'TYPE_VOICE' ? '📞 Voice' : '💬 Chat'}
                        </span>
                        {c.unread > 0 && (
                          <span style={{ fontSize: 12, fontWeight: 700, padding: '2px 8px', borderRadius: 10, background: 'rgba(245,101,101,0.1)', color: '#f56565' }}>
                            {c.unread} unread
                          </span>
                        )}
                      </div>
                    ...
                    {c.lastMessage && (
                      <div style={{ background: 'var(--bg3)', borderRadius: '0 8px 8px 8px', padding: '8px 12px', fontSize: 14, color: 'var(--text2)', lineHeight: 1.5, borderLeft: '2px solid #5b7fff' }}>
                    ...
                    {data.calls.map((call: any) => (
                      <tr key={call.id} style={{ borderBottom: '1px solid var(--border)' }}>
                        ...
                        <td style={{ padding: '10px 16px' }}>
                          <span style={{ fontSize: 12, fontWeight: 700, padding: '2px 8px', borderRadius: 10, background: call.direction === 'inbound' ? 'rgba(34,211,160,0.1)' : 'rgba(91,127,255,0.1)', color: call.direction === 'inbound' ? '#22d3a0' : '#5b7fff' }}>
                            {call.direction === 'inbound' ? '↙ Inbound' : '↗ Outbound'}
                          </span>
                        </td>
                        <td style={{ padding: '10px 16px' }}>
                          <span style={{ fontSize: 12, fontWeight: 700, padding: '2px 8px', borderRadius: 10, background: 'rgba(34,211,160,0.1)', color: '#22d3a0' }}>
                            {call.status}
                          </span>
                        </td>
                      </tr>
                  ...
                  <tbody>
                    {data.opportunities.map((opp: any) => (
                      <tr key={opp.id} style={{ borderBottom: '1px solid var(--border)' }}>
                        ...
                        <td style={{ padding: '10px 16px' }}>
                          <span style={{ fontSize: 12, fontWeight: 700, padding: '2px 8px', borderRadius: 10, background: opp.status === 'won' ? 'rgba(34,211,160,0.1)' : opp.status === 'lost' ? 'rgba(245,101,101,0.1)' : 'rgba(91,127,255,0.1)', color: opp.status === 'won' ? '#22d3a0' : opp.status === 'lost' ? '#f56565' : '#5b7fff' }}>
                            {opp.status}
                          </span>
                        </td>
                      </tr>
